...
- Identity for the session's own bash commands is not this package's job: pi sets `PI_SESSION_ID` per command, and on this rig [pi-quiet](../pi-quiet) copies it to `AGENT_SESSION_ID`, so a rig without pi-quiet has no `AGENT_SESSION_ID` in shell commands.

...
- `/bang on | off | status` — toggle at runtime. Default: on (installing the package is the opt-in).
...
`node --test src/*.test.ts` — pure skip/text/delivery/toggle rules, plus the wired behavior against a fake pi with a manual timer queue: nudge-after-timer ordering, steer-vs-followUp, `!!`/abort/disabled skips, exec passthrough, contained `sendMessage` throw, stale-ctx fallback.
